...
type UploadSignalMap = HashMap<String, watch::Sender<UploadSignal>>;
...
enum UploadError {
    ...
    #[error("文件哈希区间无效：start={start}, end={end}")]
    InvalidHashRange { start: u64, end: u64 },
...
fn io_error(action: &'static str, path: impl Into<String>, source: std::io::Error) -> UploadError {
    UploadError::Io {
        action,
        path: path.into(),
        source,
...
fn message_error(action: &'static str, detail: impl ToString) -> UploadError {
    UploadError::Message {
        action,
...
fn upload_signal_registry() -> UploadResult<MutexGuard<'static, UploadSignalMap>> {
    UPLOAD_SIGNALS
        .lock()
...
pub async fn compute_file_hash(file_path: String) -> Result<FileHashResult, String> {
    info!("[上传][哈希] 开始计算文件哈希 path={}", file_path);
    compute_file_hash_impl(file_path)
        .await
        .map_err(|err| err.to_string())
}

async fn compute_file_hash_impl(file_path: String) -> UploadResult<FileHashResult> {
...
pub async fn compute_partial_sha1(
    ...
    if end < start {
        return Err(UploadError::InvalidHashRange { start, end }.to_string());
    }

    compute_partial_sha1_impl(file_path, start, end)
        .await
        .map_err(|err| err.to_string())
}

async fn compute_partial_sha1_impl(file_path: String, start: u64, end: u64) -> UploadResult<String> {
    tokio::task::spawn_blocking(move || {
        let mut file =
            std::fs::File::open(&file_path).map_err(|e| io_error("打开文件", &file_path, e))?;
...
pub async fn upload_to_oss(
    ...
    info!(
        "[上传][{}] 开始上传 path={} bucket={} object={} resume_oss_id={:?}",
        upload_id, file_path, bucket, object, oss_upload_id
    );
    let (tx, rx) = watch::channel(UploadSignal::Running);
    {
        let mut signals = upload_signal_registry().map_err(|err| err.to_string())?;
    ...
    match &result {
        Ok(_) => info!("[上传][{}] 上传完成", upload_id),
        Err(e) => error!("[上传][{}] 上传失败: {}", upload_id, e),
    }
...
pub fn pause_upload(upload_id: String) -> Result<(), String> {
    ...
    if let Some(tx) = signals.get(&upload_id) {
        ...
        warn!("[上传][{}] 暂停失败：未找到上传任务", upload_id);
...
pub fn cancel_upload(upload_id: String) -> Result<(), String> {
    ...
    if let Some(tx) = signals.get(&upload_id) {
        ...
        warn!("[上传][{}] 取消失败：未找到上传任务", upload_id);
        Err(UploadError::UploadNotFound(upload_id).to_string())
    }
}
...
async fn upload_file_impl(
    ...
    info!(
        "[上传][{}] 使用分片上传 part_size={}MB total_parts={}",
        upload_id,
        part_size / 1024 / 1024,
        total_parts
    );
    ...
    for (part_num, range) in &ranges {
        ...
        if let Some(deadline_ms) = token_deadline_ms {
            ...
            if now_ms >= deadline_ms {
                warn!("[上传][{}] STS 凭证即将过期，终止本次上传", upload_id);
            ...
            match signal {
                UploadSignal::Paused => {
                    ...
                    loop {
                        rx.changed()
                            .await
                            .map_err(|e| message_error("等待上传控制信号", e))?;
...
pub fn resume_upload(upload_id: String) -> Result<(), String> {
    info!("[上传][{}] 请求恢复", upload_id);
    let signals = upload_signal_registry().map_err(|err| err.to_string())?;
    if let Some(tx) = signals.get(&upload_id) {
        tx.send(UploadSignal::Running)
            .map_err(|e| message_error("发送恢复信号", e).to_string())?;
        Ok(())
    } else {
        warn!("[上传][{}] 恢复失败：未找到上传任务", upload_id);
        Err(UploadError::UploadNotFound(upload_id).to_string())
    }
}
...
pub async fn scan_directory(dir_path: String) -> Result<Vec<LocalFileInfo>, String> {
    scan_directory_impl(dir_path)
        .await
        .map_err(|err| err.to_string())
}

async fn scan_directory_impl(dir_path: String) -> UploadResult<Vec<LocalFileInfo>> {
    tokio::task::spawn_blocking(move || {
...
fn scan_dir_recursive(dir_path: &str, result: &mut Vec<LocalFileInfo>) -> UploadResult<()> {
    let entries =
        std::fs::read_dir(dir_path).map_err(|e| io_error("读取目录", dir_path, e))?;
