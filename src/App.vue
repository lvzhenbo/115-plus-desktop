<script setup lang="ts">
  ...
  const { init: initProxyManager } = useProxyManager();
  ...
  onMounted(async () => {
    // 初始化下载/上传/代理管理器（幂等，设置事件监听 + 同步后端设置）
    await Promise.all([initDownloadManager(), initUploadManager(), initProxyManager()]);
